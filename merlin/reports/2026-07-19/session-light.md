...
**Target usage:** light — a day with less available capacity. Estimated queue: ~2.6h.
...
1. **Investigate the checked -> sent drop-off (436 leads lost, 30% conversion) before building anything new** (~0.5h)
...
2. **Unstick the "checked" stage — 436 leads have sat there with zero movement since the previous run (623 reached, unchanged)** (~0.5h)
...
4. **Unstick the "drip_d1_sent" stage — 17 leads have sat there with zero movement since the previous run (18 reached, unchanged)** (~0.5h)
