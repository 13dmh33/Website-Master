...
// instagram-publish.js — direct posting via the Instagram Graph API.
// Adapted from Miley's proven module; credentials are TATAS_IG_TOKEN +
// TATAS_IG_USER_ID (never Miley's).
//
// Two-call flow per Meta docs: create a media container from a PUBLIC image
// URL, poll until Meta finishes fetching it, then publish. Carousels build one
// child container per slide, then a parent CAROUSEL container.
//
...
//   - Rate limit: 100 API-published posts per 24h per account.
...
function isConfigured() {
  return !!(process.env.TATAS_IG_TOKEN && process.env.TATAS_IG_USER_ID);
}

function getToken()     { return process.env.TATAS_IG_TOKEN; }
function getAccountId() { return process.env.TATAS_IG_USER_ID; }
...
async function publishPost({ imageUrls, caption }) {
  if (!isConfigured()) throw new Error('Instagram API not configured (TATAS_IG_TOKEN / TATAS_IG_USER_ID).');
