...
- Contact Scraper: ✅ scripts/contact-scraper.js — zero-API email discovery from has-website leads; additive state.json writes; resets to 'scouted' for re-diagnosis; **on branch feature/contact-page-scraper (not merged)**. **`--deep` (2026-07-10, branch `feat/contact-scraper-deep`, not merged):** opt-in deep crawl (up to 3 contact/about/team/staff/meet/our-story pages, robots-aware, ~1 req/sec, 10s timeout) that also fills empty `phone`, `contact_name`, and `socials` fields additively and syncs them to the Google Sheet AllContacts tab (via new shared `scripts/lib/google-sheets.js`, factored out of sheet-log.js). Default (no-flag) behaviour is byte-for-byte unchanged; re-diagnosis still triggers on a new email only.
- Directory Scraper: ✅ scripts/directory-scraper.js — **new isolated enrichment agent (2026-07-12, branch `claude/trade-directory-enrichment-iq4nm3`, not merged)**; runs conceptually between Scout/Contact-Scraper and Enricher ("free before paid"). Looks each queued lead up on trade directories (**BBB implemented**; Yelp = phase 2 stub; Angi/HomeAdvisor skipped by design — login walls), matches profile→lead on **≥2 of 3 signals (name / city / phone), confidence logged, never guesses**, and additively fills empty `email`/`phone`/`website`/`socials` on the lead record + adds `directorySource`/`directoryProfiles[]`/`directoryEnrichedAt` to state.json. Re-diagnosis triggers **only on a new email** (same contract as Contact Scraper). **Dry-run is the default** — pass `--live` (or `DIRECTORY_LIVE=1`) to write. Polite crawler (1 req/3–5s per domain, rotating UA, 10s timeout, optional Playwright render for anti-bot search). Socials come from the shared `scripts/lib/social-extractor.js` (one extractor, also used by contact-scraper `--deep` — re-exports contact-scraper's proven primitives, zero mod to it). Socials reach the CRM via the next `sheet-log.js` run (AllContacts gained a `socials` column, col L, sourced from lead records). Flags: `--live` / `--limit N` / `--source bbb` / `--min-confidence N` / `--no-render`. **Must run on Mac** (BBB blocked from container proxy). **Contamination fix (2026-07-12):** `parseBbbProfile` now takes email/socials/phone **only** from the matched business's JSON-LD node (LocalBusiness family, disambiguated by name), never page-wide — previously it scraped the BBB page footer and would attach BBB's own socials + `complaints@bbb.org` to the lead. 21 unit tests (matching + BBB parsers incl. contamination regression + shared extractor) passing.
- Owner Resolver: ✅ scripts/owner-resolver.js — **new isolated agent (2026-07-12, branch `claude/trade-directory-enrichment-iq4nm3`, not merged)**; names the human OWNER of a no-website lead from FREE public records (the corpus is 0/322 owner names, 2/322 sites — so owner data can't come from the web/permutation; it comes from off-site records keyed on business name + city, which we already have). Framework + pluggable per-source **adapters** (each defines `eligible(rec)` + `lookup`; result = `{owner_name?, extra?, confidence, …}`). **Two adapters implemented, both free Socrata JSON APIs on `data.colorado.gov` (field names verified vs Socrata API Foundry, not guessed):** (1) **`co-sos`** = CO SOS business registry (`4ykn-tg5h`) → owner = the registered agent **only when it's an individual** (commercial RA services Northwest/LegalZoom/CSC/… skipped). (2) **`co-dora`** = CO DORA professional/occupational licenses (`7s5z-vewr`) for **CO plumbing/electrical leads** → matches the firm on `entityName`+city, confirms **licensure** (`license_number`/`license_status`/`licensed_trade`/`license_verify_url`, written additively) and names the **owner** from the record's `firstName`/`lastName` when present; **trade-gated** (a plumber lead never matches an electric firm) and skips entity-less individual records. Both datasets have **no phone/email**, so matching is a strict **name AND city** (2-of-2) via directory-scraper's gate + an **ambiguity guard** (same-name entities naming different owners → skip, never guess). Additive-only writes (`contact_name` + `owner_*` + licensure); **does NOT re-diagnose** (a name doesn't change channel). Dry-run default (`--live`/`OWNER_LIVE=1`); `--source co-sos|co-dora|az-roc`; ~1 req/sec; optional `SOCRATA_APP_TOKEN`. **(3) `az-roc`** = Arizona Registrar of Contractors — AZ has **no open API**, only a Salesforce Experience portal (`azroc.my.site.com`), so this adapter renders it with **Playwright** and text-parses owner/qualifying-party + licensure (AZ profiles carry a **phone** → stronger 3-signal match). The matching/owner/guardrail logic is fully unit-tested; **the retrieval + text-parse layer is a flagged best-effort whose field layout must be confirmed on a live Mac run** (capture one real profile to harden the regexes). Feeds personalization (caller.js/DMs/drip = "Hey John") + email-permuter for has-website leads. Planned adapters: fl-sunbiz, tx-*. **Must run on Mac** (container proxy blocks data.colorado.gov + azroc.my.site.com; az-roc also needs `playwright`). 25 unit tests. **62 CO + 4 AZ of 288 queued leads** in scope today.
- Email Permuter: ✅ scripts/email-permuter.js — **new isolated agent (2026-07-12, branch `claude/trade-directory-enrichment-iq4nm3`, not merged)**; the no-Apollo replacement for the EMAIL step. Takes a lead's owner name (`contact_name`) + website domain → ranked B2B email patterns (`first.last@`, `first@`, `flast@`, …) → MX/DNS gate → optional free-tier verify API (`EMAIL_VERIFY_URL`, e.g. Email Hippo ~100/day). State-agnostic. **Conservative by design:** never promotes a *guess* to `lead.email` or re-diagnoses — stages ranked guesses in `email_candidates` for review; promotes to `email`+re-diagnose ONLY when `--verify` (mailbox confirmed) or `--trust-mx` (top MX-gated pattern, corporate domains only). Skips freemail/no-MX/no-name/no-domain. Flags: `--live`/`--verify`/`--trust-mx`/`--limit`. 8 unit tests. **Coverage reality (measured 2026-07-12): current corpus has 0/322 owner names and only 2/322 domains** — this agent is starved until owner-name + has-website coverage exists (run contact-scraper `--deep` on Mac, or build the registry/license owner-resolver). True SMTP verification is out (port 25 blocked in container + most Mac ISPs; catch-all ~18%; gmail obscured) → MX-gate + free-tier API is the realistic free path.
...
- Directory Scraper (scripts/directory-scraper.js) — BBB/directory fetches blocked by container proxy (same as Scout)
- Owner Resolver (scripts/owner-resolver.js) — data.colorado.gov (co-sos/co-dora) + azroc.my.site.com (az-roc, Playwright) blocked by container proxy ("Host not in allowlist")
...
- 2026-07-12: Owner-contact enrichment suite added (branch `claude/trade-directory-enrichment-iq4nm3`) — reframed around finding the business OWNER + contact without Apollo. Measured the corpus (0/322 owner names, 2/322 websites) → owner data can't come from the web/permutation for no-website leads; it comes from off-site public records keyed on business name + city. Built: (a) `owner-resolver.js` framework + `co-sos` (CO SOS registry, free Socrata `4ykn-tg5h`) + `co-dora` (CO DORA licenses, free Socrata `7s5z-vewr`; licensure + owner-from-contractor-record, trade-gated) adapters — additive `contact_name`/`owner_*`/licensure, strict name+city match, RA-service + ambiguity guards, no re-diagnosis; (b) `email-permuter.js` — no-Apollo name+domain→ranked patterns→MX gate→optional free-tier verify, conservative (stages guesses, promotes only on --verify/--trust-mx). Also fixed the Directory Scraper BBB contamination bug (trusted-source JSON-LD only, no page-wide scrape of the directory's own socials/email). All Socrata field names verified vs the API Foundry docs (not guessed). 65/65 tests green. All new agents run on Mac (container proxy blocks data.colorado.gov + BBB).
- 2026-07-12: Directory Scraper built (branch `claude/trade-directory-enrichment-iq4nm3`, not merged) — new isolated enrichment agent `scripts/directory-scraper.js` that sits between Scout/Contact-Scraper and Enricher (free contact data before paid Apollo). Looks each queued lead up on BBB, matches profile→lead on ≥2-of-3 signals (business name token-overlap / city / phone) with logged confidence (never guesses / skips below threshold — wrong-business contamination is the main risk), and additively fills empty `email`/`phone`/`website`/`socials` on the lead record; re-diagnoses only on a new email (same contract as Contact Scraper). Dry-run is the DEFAULT (`--live`/`DIRECTORY_LIVE=1` to write). Polite crawler: 1 req/3–5s per domain, rotating UA, 10s timeout, optional Playwright render for anti-bot search pages (graceful skip if not installed). Extracted the shared `scripts/lib/social-extractor.js` (ONE regex/URL extractor used by both this agent and contact-scraper `--deep` — it re-exports contact-scraper's proven primitives, so contact-scraper is untouched per the pipeline-integrity rule). `sheet-log.js` AllContacts gained a `socials` column (col L, sourced from lead records, single Sheet writer — no rebuild/append conflict). Yelp deferred to phase 2 (heavy anti-bot); Angi/HomeAdvisor skipped (login walls). BBB fetches are blocked by the container proxy → **run on Mac**; built + verified via 19 fixture-based unit tests (matching logic + BBB search/profile parsers + shared extractor), existing Scout tests still green, no new deps.
