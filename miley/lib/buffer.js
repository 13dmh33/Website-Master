...
// videoPath:  optional local .mp4 (reels) — posted as a video with the first
//             image as its thumbnail; falls back to an image post on failure
...
async function schedulePost({ imagePaths, videoPath, caption, scheduledAt }) {
  ...
  // For a reel this becomes the thumbnail (first beat frame).
  let photoUrl = null;
  ...
  // upload the reel video if one was rendered
  let videoUrl = null;
  if (videoPath) {
    try {
