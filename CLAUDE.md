...
- Enricher: ✅ scripts/enricher.js — Apollo.io People Match, 200 credit/mo cap; finds owner email; upgrades sms→email. **Hit/miss stats now persisted (2026-07-17):** each non-dry run folds found/no_match/errors/briefs_upgraded into a lifetime `stats` block in `config/enricher-config.json` and reports the running match rate (found ÷ resolved lookups). Scout/Outscraper still returns email empty ~95% of the time, so Enricher remains the main systematic email source — now with a measured success rate.
...
- Reply Classifier: ✅ scripts/reply-classifier.js — keyword intent classifier, zero API cost; wired into **both** the email Reply Agent and (2026-07-17) the SMS Webhook
- Dashboard: ✅ scripts/dashboard.js — terminal pipeline view, --leads/--drip, color-coded
- Webhook: ✅ scripts/webhook.js — Twilio inbound SMS, HMAC-SHA1; **Mac only**. **Classifier-routed (2026-07-17):** inbound SMS now goes through reply-classifier — opt-outs/"not interested"/"already have one" → unsubscribed, objections → on_hold (no booking reply), interest/questions/short-ok → positive, detected auto-reply → ignored. Fixes the old "anything not STOP = positive" bug that fired booking replies at rejections + out-of-office autoresponders. Server wrapped in a main-guard (`classifyInbound` exported); 12 unit tests (`scripts/webhook.test.js`).
...
- 2026-07-17 (overnight): Webhook classifier wiring + Enricher stats + doc fixes — webhook.js now routes inbound SMS through reply-classifier (opt-outs→unsubscribed, objections→on_hold, interest→positive, auto-reply→ignored), fixing the "anything not STOP = positive" bug that booked replies at rejections/OOO; server behind a main-guard, `classifyInbound` exported, 12 tests. enricher.js persists a lifetime `stats` block (match rate) via pure `mergeStats`, 5 tests. Corrected stale docs: Miley Reels engine is merged to main. 38 unit tests green across reply-agent/webhook/enricher. All on branch `claude/email-agent-scope-audit-ku4pkc`.
...
- [x] **Persist Enricher hit/miss stats (done 2026-07-17)** — `scripts/enricher.js` now folds each non-dry run into a lifetime `stats` block in `config/enricher-config.json` (found/no_match/errors/briefs_upgraded + running match rate); pure `mergeStats` helper, 5 unit tests. Apollo's real success rate is now measured instead of guessed.
