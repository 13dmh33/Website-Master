...
function staticCandidates() {
  return [
    ...
    {
      id: 'miley_catalog_pull_audit',
      label: 'Audit why Miley (Techs4Tatas) is not pulling all catalog items, and fix the pull',
      category: 'code_build',
      executor: 'claude_code',
      revenueProximity: 3,
      buildVolume: 3,
      effortHours: 2,
      why: 'Miley generates content for a real product business, but its catalog pull is incomplete — scripts/scrape-catalog.js gets a 403 from techs4tatas.printify.me on every server-side fetch path (anti-bot, not a container egress issue), so the product set it writes posts about is partial or stale. Establish what is actually missing and why before judging post quality: content graded against an incomplete catalog will point at the wrong fixes. Sequence matters — this comes before miley_post_quality_audit.',
    },
    {
      id: 'miley_post_quality_audit',
      label: 'Audit and improve Miley (Techs4Tatas) post quality once the catalog pull is complete',
      category: 'code_build',
      executor: 'claude_code',
      revenueProximity: 3,
      buildVolume: 4,
      effortHours: 3,
      why: 'Review what the pipeline is actually producing — hooks, captions, visual variety, product coverage, whether the brand voice holds across the rotation — and improve the generator against real output rather than assumptions. Deliberately scheduled after miley_catalog_pull_audit: grading posts while the product data feeding them is incomplete would mistake a data gap for a content problem.',
    },
    {
      id: 'dmarc_tighten',
