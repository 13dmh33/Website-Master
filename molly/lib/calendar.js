...
// calendar.js — evaluates templates/calendar.json against a given week, for
// Trevo Advisors (Molly). Called from lib/planner.js. Molly has no seasonal
// campaign mode (no September ramp / October daily cadence like Miley), so
// every entry that matches is folded into the week's angle context rather
// than overriding a fixed post slot — see planner.js buildWeekPlan().
// Returns the calendar entries whose rule matches ANY day in the Mon-Sun
...
function matchesRule(rule, days) {
  switch (rule.type) {
    ...
    case 'monthly_day': {
      return days.some(d => d.getDate() === rule.day);
    }
    case 'explicit_range': {
