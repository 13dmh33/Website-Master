...
- Contact Scraper: ✅ scripts/contact-scraper.js — zero-API email discovery from has-website leads; additive state.json writes; resets to 'scouted' for re-diagnosis; **merged to main** (verified 2026-07-24). **`--deep` (2026-07-10, merged to main):** opt-in deep crawl (up to 3 contact/about/team/staff/meet/our-story pages, robots-aware, ~1 req/sec, 10s timeout) that also fills empty `phone`, `contact_name`, and `socials` fields additively and syncs them to the Google Sheet AllContacts tab (via new shared `scripts/lib/google-sheets.js`, factored out of sheet-log.js). Default (no-flag) behaviour is byte-for-byte unchanged; re-diagnosis still triggers on a new email only.
...
- Reply Agent: ✅ scripts/reply-agent.js (branch `claude/email-agent-scope-audit-ku4pkc`, not merged) — reads inbound Zoho replies from **known leads only**, strips quoted history/signature (plain + HTML-only fallback), guardrails opt-outs/OOO via reply-classifier (no draft; marks unsubscribed), drafts a **GAP-selling** reply with **Claude Haiku** (fed the diagnosis/review data + original pitch → names the current-state gap, ties to impact, advances with one question/CTA), and **appends a threaded draft to the Zoho Drafts folder** for human review — **nothing auto-sent**. Marks leads `reply_drafted` (not `positive`) so mobile.js never also canned-auto-replies. Absorbs poller's IMAP/dedup/OOO/lead-match (Option A). Efficiency: two-phase fetch (envelope scan → source only for known-lead replies) + UID high-water-mark (`last_uid`), batched state writes; auto-detects the special-use `\Drafts` folder and fails fast before spending if none found; RFC-2047-encoded subjects. $1/mo Haiku cap (`config/reply-agent-config.json`); full signature block; **Mac only** (Zoho IMAP), cron 4×/day. Requires `npm install imapflow mailparser`. 15 helper unit tests passing (`scripts/reply-agent.test.js`).
...
- 2026-07-24: CAN-SPAM compliance shipped — `config/compliance.json` + `scripts/lib/compliance.js` append a required footer (ad identification #3, physical postal address #4 = Dave's home address, reply-"stop" opt-out #5) to every email at send time in pitcher.js + drip.js; `assertEmailCompliant()` hard-blocks sending until all three are present. Also `scripts/lib/do-not-contact.js`: a CONTACT-keyed (email/phone/domain) global suppression store so opt-outs survive across campaigns even when a business is re-scraped under a new lead_id — enforced at ingestion (sheet-import) and every send lane; self-heals from state.json via syncFromState().
- **2026-07-26: THE Scout has-website bug fixed (why "Scout finds no email leads" all along).** Outscraper `/maps/search-v3` returns the website under `website` (Scout read `site`) and address under `address` (Scout read `full_address`), so `r.site` was always undefined and EVERY has-website lead was discarded as "no real site" (110/110 Austin plumbers that all had sites). Fix: dropped the wrong `fields` filter, normalize `website→site`/`address→full_address` in `scout-shared.normalizeOutscraperRows`, applied AFTER the async-poll flatten (the nested `[[...]]` shape defeated a first attempt). Second gap: has-website "needs-email" leads were written only to `leads-web/` where contact-scraper never looked — now mirrored into `leads/` + registered `scouted` so the pipeline actually flows. End-to-end result: Austin run found 75 sites → contact-scraper harvested 24 emails free → 16 approved CAN-SPAM-compliant email leads. Regression tests in `scripts/test/scout-outscraper-fields.test.js`.
- 2026-07-26: Checker state-gate fix — `checker.js` only re-checks leads that are actually `diagnosed` (new exported `isCheckable()`), not any brief with `checker_approved=false`. The old behavior re-swept settled leads and clobbered an opt-out's status back to `checked` (twice). Also `scripts/lib/lead-quality.js` (isPlaceholderEmail / isNonProspectBusiness) wired into contact-scraper so supply houses (Ferguson/Josco/Winsupply...) and template emails (`info@mysite.com`) are auto-excluded instead of filtered by hand. Tests: `checker-gate.test.js`, `lead-quality.test.js`.
- **2026-07-27: Molly automated — cloned Miley's queue-first pipeline.** Audit found the pipeline (`agents/researcher|generator|designer|scheduler.js`, `lib/brand-validator.js`) was already fully built, just never scheduled and sitting on two live bugs: the evergreen fallback bank still pointed at the 2026-07-18-quarantined (rule-violating) content — meaning with no `SERPAPI_KEY` set, every weekly run would have produced zero postable posts — and `generator.js`'s own `trevo_found` Claude prompt instructed stating the $65/mo price, which `brand-validator.js` then guaranteed-rejected. Fixed: `templates/evergreen.json` rewritten from scratch (20 posts, 5/format, all pass `brand-validator.js`, deliberately no hard statistics since this runs unattended); `lib/store.js`'s `getUnusedEvergreen()` now picks one post per needed format instead of a FIFO slice (was silently mismatching e.g. a carousel post's slide-formatted body onto the caption slot); the reel script's `HOOK`/`BODY`/`CTA` labels re-cased to `Hook`/`Body`/`Cta` (all-caps was tripping the sentence-case check on every reel, generated or evergreen). Added `.github/workflows/molly-weekly-pipeline.yml` (Mon 6am MT) + `molly-weekly-analytics.yml` (Sun 10pm MT), `FORCE_QUEUE=1` default — nothing auto-posts. Verified end-to-end with a real run: 4 posts generated, zero brand-validator rejections, real images rendered, real `output/queue/preview-2026-07-27.html` produced. Pending: Buffer classic token, merge to `main`, GitHub Actions secrets — see `molly/CLAUDE.md`'s "Activation checklist."
- **2026-07-28: Molly brought to parity with Miley's more built-out engine** — ported 5 capabilities Miley had that Molly didn't: (1) **real reel videos** — the Sunday reel slot rendered only a static thumbnail before; now renders an actual vertical 1080×1920 `.mp4` via `lib/reel.js` (ported unchanged from Miley — brand-agnostic) + new Trevo-branded reel-frame renderers in `lib/canvas-render.js`, with `card` and `kinetic_karaoke`/`kinetic_punch` styles rotating weekly, silent, $0 marginal cost; `lib/buffer.js` gained video upload so it actually posts as a reel, not an image. Reel content reshaped from a timestamped voiceover script to 3 short on-screen beats (`hook`/`body`/`cta` + separate `caption`) — `templates/evergreen.json`'s 5 reel posts rewritten to match, re-verified against `brand-validator.js`. (2) **`lib/planner.js`** — centralizes weekly rotation (week number, niches, the `trevo_found` demo-vs-agent pick, new reel-style rotation) that used to be scattered inline in `generator.js`. (3) **`lib/calendar.js` + `templates/calendar.json`** — date-specific observances (Skilled Trades Day, Small Business Week, seasonal windows) folded into weekly prompts as context, never overriding Molly's fixed 4-post schedule. (4) **`lib/sentiment.js`** — comment/DM sentiment classification wired into `agents/analyst.js`, runs regardless of Instagram-insights config. (5) **Test coverage** — 5 new test files, 47 tests passing total. Full detail: `molly/docs/reels.md`, `molly/docs/review-workflow.md` (new), `molly/CLAUDE.md`'s "Reels, planner, calendar, sentiment" section.
...
- [x] **Contact-scraper built + merged to main** (base + `--deep`, verified 2026-07-24) — run on Mac after Scout `--mode has-website` to discover emails for free.
...
- [x] **Persist Enricher hit/miss stats** (done 2026-07-26) — `scripts/enricher.js` now accumulates `stats` (attempted/found/no_match/errors/runs + lifetime `hit_rate_pct`) into `config/enricher-config.json` via `updateEnricherStats()`, and prints the lifetime match rate each run. Match rate = found / (found + no_match); transient errors excluded. 5 tests in `scripts/test/enricher-stats.test.js`.
...
- [ ] **Campaign-exit on reply (scoped 2026-07-17, NOT built — stopped per owner):** when `reply-agent.js` drafts a reply or logs an opt-out, flag the prospect out of the outreach campaign, halt further drip/pitcher sends, and record a structured `campaign` exit block (`{status:'exited', reason:'replied'|'opted_out', exit_step, exit_channel, exited_at}`) on the sent record + `state.json`. **Live gap it closes:** `drip.js` (lines 133/185) only skips `positive`/`unresponsive`, so leads marked `reply_drafted`/`unsubscribed` by the reply-agent would still receive drip follow-ups; `sheet-log.js` (line ~133) doesn't recognize `reply_drafted` so they read "sent" on the CRM sheet. Files to touch: reply-agent.js (write exit), drip.js + pitcher.js (suppress exited), sheet-log.js (reflect exit + changelog). Design decision pending: reflect to the Google Sheet lazily via the next sheet-log run (recommended) vs. immediately from reply-agent.
