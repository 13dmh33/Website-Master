...
- Directory Scout: ✅ scripts/directory-scout.js — public-directory lead sourcing (Yellow Pages / BBB / state licensing-board CSV) to solve the has-website email wall (GMB omits the website URL ~65% of the time). Shared lib + per-source adapters (`scripts/lib/directory-scout.js` + `dir-yellowpages.js` / `dir-bbb.js` / `dir-licensing.js`); Scout-shaped output feeds contact-scraper→Diagnoser unchanged; dedup by lead_id+domain; HVAC blocked; `--dry-run`/`--csv`/`--limit`/`--pages`/`--html`/`--save-html`. 20 tests (unit + offline E2E chain). **Mac only** (container egress 403s all external hosts). **on branch claude/weekly-email-revenue-plan-4ysmqx (not merged)**
...
- 2026-07-01: Root-caused the has-website email wall — repeated Scout `--mode has-website` runs (Denver/Houston/Atlanta plumber+electrician, incl. `--budget 1` = 101 raw) returned 0 usable leads because Google Maps omits the website URL on ~65% of listings (`No real site` dominates every breakdown), so contact-scraper had nothing to visit. Built **Directory Scout** as a better top-of-funnel: Yellow Pages (exposes the contractor's own site URL — the field GMB drops), BBB (JSON search API), and state licensing-board CSV import. Shared lib + thin adapters + one runner, Scout-shaped output, 20 tests. Licensing CSV path verified end-to-end in-container (real write + dedup + idempotency); YP/BBB parsers proven via an offline E2E integration test (YP HTML → lead → contractor-site HTML → email) but NOT against live markup — container egress 403s every external host (curl + proxy-status + WebFetch all confirm; policy denials not retried). Added offline `--html`/`--save-html` debug mode + a loud self-check (HTTP 200 but 0 parsed → selectors changed, here's the one-command fix path) so the first Mac run can't silently fail.
