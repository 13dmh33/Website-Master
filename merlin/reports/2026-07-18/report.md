# Merlin nightly report — 2026-07-18
...
**Clear the 458-lead checked-but-unsent backlog — run Pitcher daily (already scheduled) or raise the daily cap if speed matters more than pacing**

458 leads are already Checker-approved and waiting — every one of them is closer to paid than any lead that doesn't exist yet. At the current 30/day cap this takes 16 days; zero new code required either way. This is currently the single largest lever available.
...
- 10 local branches — 2 stale (14+ days), 5 unpushed.
- Safe to delete (stale and already merged): claude/miley-techs4tatas.
- Untracked top-level directories: job-hunter, leads-web, nora — confirm each is intentional.

## Funnel state

Biggest drop-off: checked -> sent, 458 leads lost (26.5% conversion).
  scouted: 636
  diagnosed: 625
  checked: 623
  sent: 165
...
1. **[RECOMMENDED]** Clear the 458-lead checked-but-unsent backlog — run Pitcher daily (already scheduled) or raise the daily cap if speed matters more than pacing (score 27.0, revenue 9/10, build 0/10, ~0.1h)
   458 leads are already Checker-approved and waiting — every one of them is closer to paid than any lead that doesn't exist yet. At the current 30/day cap this takes 16 days; zero new code required either way. This is currently the single largest lever available.
...
3. Investigate the checked -> sent drop-off (458 leads lost, 26.5% conversion) before building anything new (score 21.0, revenue 7/10, build 0/10, ~0.5h)
